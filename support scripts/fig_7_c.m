...
color_index = {[188,189,220]./256,[128,125,186]./256,[74,20,134]./256}; %orange to dark red with increasing alpha
data_index = {'Hct20','m_3','Hct60'};
dt = 1/6;
...
figure('name','Figure 7: Impact of hematocrit on NO dynamics','NumberTitle','off',...
    'position',pos,'color','w');

%% plot baseline vasodilation
subplot(2,3,1), hold on
plot(data.fig_7.baseline.Hct(:,1).*100,data.fig_7.baseline.Hct(:,2),'-sk','LineWidth',3,'MarkerFaceColor',[0 0 0])
plot(data.fig_7.baseline.Hct_feedback(:,1).*100,data.fig_7.baseline.Hct_feedback(:,2)./3,'-sr','LineWidth',3,'MarkerFaceColor',[0 0 0])
ylabel('\DeltaGC activation (%)')
xlabel('Hematocrit (%)')
title('increased hematocrit decreases vessel diameter')
%ylim([-5 5])
xlim([0 100])
...
yyaxis right
plot(data.fig_7.baseline.Hct(:,1).*100,data.fig_7.baseline.Hct(:,2).*3,'k','LineWidth',1)
plot(data.fig_7.baseline.Hct_feedback(:,1).*100,data.fig_7.baseline.Hct_feedback(:,2),'r','LineWidth',1)
...
kk = 1;
for cfl = [1.9 2.8 5]
f = fill([10 10-cfl 10-cfl 10],[0 0 14 14],'m','Linestyle','none');
set(f,'facea',1/10)
...
figure, hold on
MAP = 100;
plot(data.fig_7.baseline.Hct(:,1).*100-45,(MAP-((100+data.fig_7.baseline.Hct(:,2).*3)/100).^4.*MAP),'-sk','LineWidth',3,'MarkerFaceColor',[0 0 0])
plot(data.fig_7.baseline.Hct_feedback(:,1).*100-45,(MAP-((100+data.fig_7.baseline.Hct_feedback(:,2))/100).^4.*MAP),'-sr','LineWidth',3,'MarkerFaceColor',[0 0 0])
ylabel('\DeltaMAP (mmHg)')
xlabel('\DeltaHematocrit (%)')
